    ...
    public class ContractSearchService : IContractSearchService
    {
        public ContractListDataModel SearchContract(ContractSearchModel searchModel)
        {
            ...
            if (!string.IsNullOrEmpty(searchModel.ContractDateTo))
            {
                ...
                query = query.Where(c => c.CreatedDateTimeO <= toDate);
            }
            ...
            var result = query
                .OrderByDescending(x => x.CreatedDateTimeO)
                .Skip(searchModel.PageIndex * searchModel.PageSize)
                .Take(searchModel.PageSize)
                .ToList();

            // 取的目前合約ID集合
            var contractIDs = result.Select(c => c.ContractID).ToList();
            ...
            foreach (var contract in result)
            {
                ...
                if (crs != null)
                {
                    contract.Parties = crs.Signers.Select(s => new PartyRefs
                    {
                        ...
                        Step = GetStep(contract.CurrentStep ?? 0, contract.IsPassStamp, s.IsInitiator, s.StampDate, s.SignatureDate),
                        IsCurrentUserCompany = (s.CompanyID == searchModel.SearchCompanyID),
                        IsInitiator = s.IsInitiator
                    });
                }
            }
            
            int GetStep(int currentStep, bool? isPassStamp, bool? isInitiator, DateTime? stampDate, DateTime? signerDate)
            {
            ...
            ContractListDataModel contractListDataModel = new ContractListDataModel
            {
                Contracts = result,
